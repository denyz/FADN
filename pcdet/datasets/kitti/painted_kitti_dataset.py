...
class PaintedKittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        self.split = self.dataset_cfg.DATA_SPLIT[self.mode]#val和test 的self.mode都是test
        # 如果painted_kitti_dataset.yaml中的DATA_SPLIT: test为‘test’则进入teting文件夹，如为val则为evaluation模式
        self.root_split_path = self.root_path / ('training' if self.split != 'test' else 'testing')

        split_dir = self.root_path / 'ImageSets' / (self.split + '.txt')
        self.sample_id_list = [x.strip() for x in open(split_dir).readlines()] if split_dir.exists() else None #testing 7518个样本
    ...
    def set_split(self, split):
        ...
        #读取imageSets文件夹下的txt文件，根据txt文件中的id来对数据集进行划分。
        split_dir = self.root_path / 'ImageSets' / (self.split + '.txt')
    ...
    def create_groundtruth_database_test(self, info_path=None, used_classes=None, split='train'):
        import torch
        ...
        for k in range(len(infos)):
            ...
            info = infos[k]
            if self.mode == 'test':
                path_str =  info['point_cloud']['velodyne_path']
                sample_idx = "".join(list(filter(str.isdigit, path_str)))
                points = self.get_painted_lidar(sample_idx)
                print(points.shape)
                print(points.dtype)
                db_path = 'testing'/ 'painted_lidar' / ('%s.npy' % sample_idx)
                if (used_classes is None) or names[i] in used_classes:
                    db_path = path_str
                    db_info = {'path': db_path, 'image_idx': sample_idx}
                ...
                for i in range(num_obj):
                    ...
                    # 将目标中心三维坐标转到原点？PCDet框架的7维检测框定义为：3D bounding box: (cx, cy, cz, dx, dy, dz, heading)，heading为方向角
...
def create_kitti_infos_original(dataset_cfg, class_names, data_path, save_path, workers=8):
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=8):
    ...
    # dataset.create_groundtruth_database(train_filename, split=train_split)
    #
    # print('---------------Start create painted evaluation database for data augmentation---------------')
    #
